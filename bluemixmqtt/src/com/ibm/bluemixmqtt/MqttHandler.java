...
public class MqttHandler implements MqttCallback {
	...
	public void connect(String serverHost, String clientId, String authmethod,
		...
		if (!isMqttConnected()) {
			...
			if (isSSL) {
				connectionUri = "ssl://" + serverHost + ":" +
				DEFAULT_SSL_PORT;
				//connectionUri = "ssl://127.0.0.1:8883";
			} else {
				connectionUri = "tcp://" + serverHost + ":" +
				DEFAULT_TCP_PORT;
				//connectionUri = "tcp://127.0.0.1:1883";
			}
			...
			try {
				client = new MqttClient(connectionUri,
			...
			if (isSSL) {
				...
				sslClientProps.setProperty("com.ibm.ssl.contextProvider",
						"SunJSSE");

				sslClientProps.setProperty("com.ibm.ssl.keyStore",
						"C:/certificates/iot_device_keystore.jks");
	...
	public void connect(String serverHost, String clientId, String authmethod,
			String authtoken, boolean isSSL, String strKeyStore, String keyPass) {
		// check if client is already connected
		if (!isMqttConnected()) {
			String connectionUri = null;
			...
			if (isSSL) {
				...
				sslClientProps.setProperty("com.ibm.ssl.keyStore", strKeyStore);
				sslClientProps.setProperty("com.ibm.ssl.keyStorePassword",
						keyPass);
				sslClientProps.setProperty("com.ibm.ssl.keyStoreType", "JKS");
				sslClientProps.setProperty("com.ibm.ssl.keyManager", "SunX509");
